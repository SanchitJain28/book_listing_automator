...
(async () => {
  ...
  let { context, page } = await initBrowser(isHeadless, "amazon_us_profile");
  ...
  for (let i = startIndex; i < inputItems.length; i++) {
    ...
    if (i > 0 && i % 500 === 0) {
      ...
      await context.close();
      const newBrowser = await initBrowser(isHeadless, "amazon_us_profile");
    ...
    while (retries > 0 && !success) {
      try {
        startSpinner(`Searching Amazon.com for ISBN ${isbn}...`);

        await page.goto(`https://www.amazon.com/s?k=${encodeURIComponent(isbn)}`, {
          timeout: 45000,
  ...
  await context.close();
  console.log(`\n🎉 Amazon US Scraper completed. Results saved to ${outputFilePath}`);
})();
