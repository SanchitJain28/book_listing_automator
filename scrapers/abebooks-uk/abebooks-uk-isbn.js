...
async function navigateAbeBooksUk(page, directUrl, targetIsbn) {
  let retries = 3;
  let targetUrl = directUrl;

  if (!targetUrl && targetIsbn) {
    targetUrl = `https://www.abebooks.co.uk/products/isbn/${encodeURIComponent(targetIsbn)}`;
  }
  ...
  while (retries > 0) {
    ...
    if (status === 429 || pageText.includes("Too Many Requests")) {
      console.log(`   🛑 AbeBooks UK 429 Rate Limit hit! Waiting 45 seconds to cooldown...`);
    ...
    if (status === 404 && !directUrl && targetIsbn) {
      targetUrl = `https://www.abebooks.co.uk/servlet/SearchResults?kn=${encodeURIComponent(targetIsbn)}`;
      retries--;
      continue;
    }
...
async function scrapeAbeBooksUkBook(page, { directUrl, isbn }) {
  const storeName = "AbeBooks UK";
  const currency = "GBP";

  const navResult = await navigateAbeBooksUk(page, directUrl, isbn);
...
(async () => {
  ...
  let { context, page } = await initBrowser(isHeadless, "abebooks_uk_profile");
  ...
  for (let i = startIndex; i < inputItems.length; i++) {
    ...
    if (i > 0 && i % 400 === 0) {
      ...
      await context.close();
      const newBrowser = await initBrowser(isHeadless, "abebooks_uk_profile");
      context = newBrowser.context;
      page = newBrowser.page;
    }

    startSpinner(`Searching AbeBooks UK for ${targetIsbn || directUrl}...`);

    try {
      const result = await scrapeAbeBooksUkBook(page, { directUrl, isbn: targetIsbn });
  ...
  await context.close();
  console.log(
    `\n🎉 AbeBooks UK Scraper completed. Results saved to ${outputFilePath}`,
  );
})();
